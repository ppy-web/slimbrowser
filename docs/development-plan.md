...
- `intent:` 与未知 scheme（包括自动重定向）都尝试交给系统处理器。`tel:`/`mailto:`/`sms:` 使用对应系统动作，`geo:`/`market:` 仍需用户确认。
...
6. **用户主导兼容**：这是个人调试浏览器，不建立域名或 HTTPS 白名单；TLS、Safe Browsing、混合内容和第三方 Cookie 不作为静默拒绝条件，WebView 无法打开时尽力委派给系统处理器。
...
普通个人浏览器以用户输入为准：裸主机默认补 HTTPS；显式 HTTP、带凭据的 URL、任意合法端口、局域网/IP、`file:`、`content:`、`data:`、`blob:` 和 `javascript:` 都优先交给 WebView 尝试。其他合法 scheme（包括自动重定向）交给系统处理器；只有空白、控制字符、无法解析的 URI 或系统没有可处理应用时显示错误。
...
- 用户输入或重定向触发的特殊协议都会尽力交给系统处理器，失败时给出可理解的错误。
- 网页点击和手工输入遵循同一安全规则。
...
- 邮件只打开邮件客户端。
- 未安装外部应用时不崩溃。
- `intent:` 和未知 scheme 在用户触发或自动重定向时都可调用已安装处理器。
...
- 常见 PDF/图片/压缩包可下载并打开。
- 文件上传可选择文件并返回网页。
- Android 权限或用户拒绝权限后网页收到系统结果，应用不崩溃。
