...
- [~] 页面标题和 favicon 同步到浏览状态（favicon 已写入应用私有存储、历史记录和列表，待实体设备验证）
...
- [x] 清除表单数据
- [x] 一键清除全部浏览数据
- [~] 隐私模式（阻止新增历史与持久会话；Cookie/缓存/存储在退出时清理，待实体设备验证）
- [~] 退出时清理隐私会话（Activity 正常结束时已接线，进程被系统直接杀死仍需平台矩阵验证）
- [x] 第三方 Cookie 策略说明
...
- [~] `HistoryRepository`：标题、URL、访问时间、隐私会话标记和 favicon URI（已在列表呈现，待实体设备验证）
- [x] 历史按今天/昨天/更早分组
- [x] 历史搜索、单项删除、全部清除
